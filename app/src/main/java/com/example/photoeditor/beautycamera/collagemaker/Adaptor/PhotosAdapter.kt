...
class PhotosAdapter(private val context: Context, private val images: List<Uri>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        return if (getItemViewType(position) == VIEW_TYPE_CAMERA) {
            val view = convertView ?: LayoutInflater.from(context).inflate(R.layout.item_camera_icon, parent, false)
            view
        } else {
            val view = convertView ?: LayoutInflater.from(context).inflate(R.layout.gallery_item, parent, false)
            val imageView: ImageView = view.findViewById(R.id.image_view_collage)
